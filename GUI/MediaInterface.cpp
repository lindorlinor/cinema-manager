...
void MediaInterface::setLimitTrailer()
{
    ...
    else if (trailer && !titoloFilmRiferimento.isEmpty() && !autoreFilmRiferimento.isEmpty())
    {
        ...
        if (filmAssociato)
        {
            ...
            dataInizio->setMaximumDate(convertToQDate(filmAssociato->getDataFineRilascio()));
            dataFine->setMaximumDate(convertToQDate(filmAssociato->getDataFineRilascio()));
        }
    }
    else if (!trailer)
    {
        QDate noLimit = QDate(7999, 12, 31);
...
QWidget *MediaInterface::addTipologiaFilm()
{ // tipologia Film
...
QWidget *MediaInterface::addTipologiaTrailer()
{ // tipologia Trailer
...
QWidget *MediaInterface::addTipologiaInserzione()
...
QWidget *MediaInterface::addTipologiaPodcast()
{ // tipologia Podcast
...
QWidget *MediaInterface::addTipologiaPuntate()
{ // tipologia Puntata
...
void MediaInterface::setLimitTabTipologia(int index)
{
...
// pulsanti
QWidget *MediaInterface::pulsanteIndietro()
{ // pulsante indietro
...
QDate MediaInterface::convertToQDate(const year_month_day &date)
{
    return QDate((int)date.year(), (unsigned)date.month(), (unsigned)date.day());
}
