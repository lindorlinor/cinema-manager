                                                                                                              ...
                                                                                                              ricerca(""), changeView(1), stackModifiche(new QStackedWidget(this))
{
    ...
    connect(stackModifiche, &QStackedWidget::currentChanged, this, [this]()
            {
        if(stackModifiche->currentIndex() == 0 ){
          emit setQMenuEnabled();
        } 
...
void SearchPanel::addPagina(QVBoxLayout *mainLayout)
{
    ...
    connect(attivita, QOverload<int>::of(&QComboBox::currentIndexChanged), this, [this](int index)
            { comboAttivita = index; preUpdate(); });
    connect(ordinamento, QOverload<int>::of(&QComboBox::currentIndexChanged), this, [this](int index)
            {comboOrdinamento = index; preUpdate(); });
...
void SearchPanel::resetSearchPanel()
{
    ...
    if (stackModifiche->currentIndex() == 1)
        emit resetPages();
...
void SearchPanel::showEditPage(Media *media)
{
    ...
    connect(editMedia, &EditMedia::tornaIndietro, this, [this, editMedia, backIndex]()
        ...
        delete editMedia; });
    connect(editMedia, &EditMedia::savedMedia, this, [this]()
            { updateMediaList(); });
}
...
void SearchPanel::showMediaView(MediaView &widget)
{
    ...
    connect(&widget, &MediaView::deleteMediaClicked, this, [this, &widget](Media *m)
            { acceptDeleteMedia(m); });
}
...
{ // per gestire il ridimensionamento, appaiono solo le icone se la widgetFiltri è troppo piccola
