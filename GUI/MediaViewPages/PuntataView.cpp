...
void PuntataView::updateMediaDetails()
{
    ...
    inizioP->setText(
        ...
        QString::fromStdString(dateToString(puntPtr->getDataInizioRilascio())) + "</span>");

    endDateLabel->setText(
        "<span style='color: #bdced3; font-weight:bold;'>Fine proiezione: </span>"
        "<span style='color: #4e7f8b;'>" +
        QString::fromStdString(dateToString(puntPtr->getDataFineRilascio())) + "</span>");

    nPubblicita->setText(
        "<span style='color: #bdced3; font-weight:bold;'>Numero di pubblicità: </span>"
        "<span style='color: #4e7f8b;'>" +
        QString::number(puntPtr->getNumeroPubblicita()) + "</span>");

    incasso->setText(
        "<span style='color: #bdced3; font-weight:bold;'>Incasso totale: </span>"
        "<span style='color: #4e7f8b;'>" +
        QString::number(puntPtr->calcolaIncasso()) + " €</span>");

    visualizzazioni->setText(
        "<span style='color: #bdced3; font-weight:bold;'>Visualizzazioni: </span>"
        "<span style='color: #4e7f8b;'>" +
        QString::number(puntPtr->getVisualizzazioni()) + "</span>");

    risoluzione->setText(
        "<span style='color: #bdced3; font-weight:bold;'>Risoluzione: </span>"
        "<span style='color: #4e7f8b;'>" +
        QString::fromUtf8(toString(puntPtr->getRisoluzione())) + "</span>");

    formato->setText(
        "<span style='color: #bdced3; font-weight:bold;'>Formato: </span>"
        "<span style='color: #4e7f8b;'>" +
        QString::fromUtf8(toString(puntPtr->getFormato())) + "</span>");

    descrizione->setText(
        "<span style='color: #bdced3; font-weight:bold;'>Descrizione: </span><br>" + QString::fromStdString(puntPtr->getDescrizione()));

    conduttore->setText(
        "<span style='color: #bdced3; font-weight:bold;'>Conduttore: </span>"
        "<span style='color: #4e7f8b;'>" +
        QString::fromStdString((puntPtr->getPodcast())->getConduttore()) + "</span>");
}
...
void PuntataView::extendMediaMessage()
{
    ...
    msgBox.setMainMessage(QString::fromStdString(
                          ...
                          QString::fromStdString("<br/>Confermando l'estensione potrebbe cambiare la data di fine rilascio del podcast associato."));
